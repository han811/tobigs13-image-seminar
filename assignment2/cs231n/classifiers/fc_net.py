...
# 참고(refference) github https://github.com/haofeixu/cs231n/blob/master/assignment2/cs231n/classifiers/fc_net.py
...
class TwoLayerNet(object):
    ...
    def __init__(self, input_dim=3*32*32, hidden_dim=100, num_classes=10,
       ...
       # first layer weights
        W1 = np.random.randn(input_dim,hidden_dim) * weight_scale     # Gaussian 정규분포로
        self.params['W1'] = W1

        b1 = np.zeros(hidden_dim)      # bias scale 0 으로 시작
        self.params['b1'] = b1
...
class FullyConnectedNet(object):
        ...
        for i in range(self.num_layers):
          ...
          # With batch normalization
            ...
            # Intermediate hidden layer
              ...
              self.params[W_i] = np.random.rand(hidden_dims[i-1], hidden_dims[i]) * weight_scale
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers-1):
          ...
          # Without batchnormalization and layer normalization : 둘 다 아닐 때
          else: out, caches[i+1] = affine_relu_forward(out,self.params[W_i], self.params[b_i])
        ...
        for i in range(self.num_layers, 0, -1):
          ...
          if i == self.num_layers:
            # dout denotes gradient of output of the last layer : dout 이 마지막 layer의 gradient
            dout, grads['W'+str(i)], grads['b'+str(i)] = affine_backward(dscores,caches[i])

          else:
            # Dropout backward except the last layer (before relu backwarod) : 마지막 layer 빼고 dropout
            if self.use_dropout:
              dout = dropout_backward(out, caches['dropout'+str(i)])
            
            # batchnorm
            if self.normalization == 'batchnorm' : 
              # 순서 drelu -> dbatchnorm -> daffine
              ...
              dout, grads['W'+str(i)], grads['b'+str(i)] = affine_forward(dfc_out,fc_cache)
